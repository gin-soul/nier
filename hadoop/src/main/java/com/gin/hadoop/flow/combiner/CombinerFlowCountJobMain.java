...
public class CombinerFlowCountJobMain extends Configured implements Tool {

    @Override
    public int run(String[] strings) throws Exception {
        //创建一个任务对象
        Job job = Job.getInstance(super.getConf(), CombinerFlowCountJobMain.class.getSimpleName());

        //打包放在集群运行时，需要做一个配置
        job.setJarByClass(CombinerFlowCountJobMain.class);
        //第一步:设置读取文件的类: K1 和V1
    ...
    //hadoop jar hadoop-0.0.1-SNAPSHOT.jar com.gin.hadoop.flow.count.CombinerFlowCountJobMain
    public static void main(String[] args) throws Exception {
        Configuration configuration = new Configuration();
        //启动一个任务
        int run = ToolRunner.run(configuration, new CombinerFlowCountJobMain(), args);
